...
const Card = ({img,projectName,className,redirect}) => {
  return (
    <div className={`${className} group overflow-hidden relative rounded-lg shadow bg-[url(assets/test.jpeg)] bg-center bg-no-repeat w-60 h-80`}>
        ...
        <h3 className="group-hover:opacity-0 group-hover:translate-y-10 transition duration-500 absolute min-h-14 w-full bottom-0 px-1.5 text-center line-clamp-2 bg-dull-black backdrop-blur-sm rounded-lg text-lg font-bold  text-white">{projectName}</h3>
